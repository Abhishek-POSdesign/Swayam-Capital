        ...
        <form id="readiness-form" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; align-items: start;">
          ...
          <div style="grid-column: 1 / -1;">
            <label style="display: block; font-size: 0.75rem; text-transform: uppercase; color: var(--text-muted); margin-bottom: 0.5rem;">
              How do I feel right now? <span style="color: var(--accent-blue);">* Required (feelings > data)</span>
            </label>
            <div id="mood-pill-container" style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
              ${[
                ...
                <button type="button" class="mood-pill ${this.selectedMood === m.id ? 'active' : ''}" data-mood="${m.id}"
                  style="padding: 0.4rem 0.8rem; border-radius: 20px; font-size: 0.825rem; cursor: pointer; border: 1px solid ${this.selectedMood === m.id ? 'var(--accent-blue)' : 'var(--border)'}; background: ${this.selectedMood === m.id ? 'rgba(88, 166, 255, 0.15)' : 'var(--bg-surface)'}; color: var(--text-primary);">
                  ${m.label}
                </button>
              `).join('')}
            </div>
    ...
    pills.forEach(p => {
      p.addEventListener('click', () => {
        ...
        pills.forEach(other => {
          ...
          other.style.borderColor = isActive ? 'var(--accent-blue)' : 'var(--border)';
