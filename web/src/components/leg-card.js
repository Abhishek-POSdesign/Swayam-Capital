...
export class LegCardComponent {
  ...
  render() {
    ...
    const delta = this.leg.delta !== undefined ? Number(this.leg.delta).toFixed(2) : '--';
    const theta = this.leg.theta !== undefined ? Number(this.leg.theta).toFixed(1) : '--';
    const vega = this.leg.vega !== undefined ? Number(this.leg.vega).toFixed(1) : '--';
    ...
    this.container.innerHTML = `
      ...
      ">
        ...
        <!-- Greeks Pill Strip -->
        <div style="display: flex; gap: 6px; flex: 1; justify-content: flex-end; align-items: center;">
          <span style="font-family: var(--font-mono); font-size: 0.72rem; color: var(--accent-lilac); background: var(--accent-lilac-tint); padding: 2px 6px; border-radius: 4px;" title="Delta">
            Δ ${delta}
          </span>
          <span style="font-family: var(--font-mono); font-size: 0.72rem; color: var(--accent-lilac); background: var(--accent-lilac-tint); padding: 2px 6px; border-radius: 4px;" title="Theta per day">
            θ ${theta}
          </span>
          <span style="font-family: var(--font-mono); font-size: 0.72rem; color: var(--accent-lilac); background: var(--accent-lilac-tint); padding: 2px 6px; border-radius: 4px;" title="Vega">
            ν ${vega}
          </span>
        </div>
