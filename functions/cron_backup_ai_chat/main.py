...
Monthly AI Chat Mirror (1st of month 04:00 IST)
...
logger = logging.getLogger("cron_backup_ai_chat")


def cron_backup_ai_chat(request):
    ...
    try:
        from swayam.services.backup_service import run_monthly_ai_backup
        success = run_monthly_ai_backup()
        return (
            json.dumps({
        ...
        logger.error("Execution error in cron_backup_ai_chat: %s", exc)
        return (json.dumps({"error": str(exc)}), 500, {"Content-Type": "application/json"})


if _has_ff:
    cron_backup_ai_chat = functions_framework.http(cron_backup_ai_chat)
