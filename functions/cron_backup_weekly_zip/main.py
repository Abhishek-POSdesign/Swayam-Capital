...
Weekly ZIP Backup (Sundays 03:00 IST)
...
logger = logging.getLogger("cron_backup_weekly_zip")


def cron_backup_weekly_zip(request):
    ...
    try:
        from swayam.services.backup_service import run_weekly_zip
        success = run_weekly_zip()
        return (
            json.dumps({
        ...
        logger.error("Execution error in cron_backup_weekly_zip: %s", exc)
        return (json.dumps({"error": str(exc)}), 500, {"Content-Type": "application/json"})


if _has_ff:
    cron_backup_weekly_zip = functions_framework.http(cron_backup_weekly_zip)
